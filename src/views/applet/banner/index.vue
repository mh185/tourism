    ...
    >
      ...
      <el-table-column label="位置">
        <template slot-scope="scope">
          <span v-if="scope.row.bannerTagEnum == 'HOME'">首页</span>
          <span v-if="scope.row.bannerTagEnum == 'SHOP'">商城</span>
          <span v-if="scope.row.bannerTagEnum == 'SHOP_RECOMMEND'">商城推荐</span>
          <span v-if="scope.row.bannerTagEnum == 'AFFAIRS'">政务服务</span>
        </template>
      </el-table-column>
      ...
      <el-form v-if="addBannerStatus" :model="form">
        ...
        <el-form-item label="是否跳转：" label-width="120px">
          <el-radio-group v-model="form.isJump" @change="change">
            ...
            <el-radio :label="3">跳转视频</el-radio>
          </el-radio-group>
        </el-form-item>
        ...
        <el-form-item v-if="form.isJump=='1'" label="关联内容：" label-width="120px">
          <el-select
            v-model="form.toType"
            v-loading="isJumpLoading"
            clearable
            remote
            placeholder="请先选择跳转类型"
          >
            <el-option
              v-for="item in contentSelect"
              :key="item.id"
              :label="item.scenicSpotName || item.name ||item.title"
              :value="item.id"
            />
          </el-select>
        </el-form-item>
        ...
        <el-form-item v-if="form.isJump=='3'" label="跳转视频：" label-width="120px">
          <el-input
...
import { pageQuery, deleteId, insert, update, upfiles, queryHotScenicSpotList, getManagerList, pageQueryTravel, pageQueryPath, pageQuerySpecialty } from '@/api/bannerApplet'
...
export default {
  ...
  data() {
    return {
      ...
      toSelect: [
        {
          value: 'pages/more/attractions/attractionsDetail/index',
          label: '景区'
        },
        {
          value: 'pages/more/deliciousFood/foodDetails/index',
          label: '美食'
        },
        {
          value: 'pages/index/gourmetSpecialty/foodDetails/index',
          label: '美食特产'
        },
        {
          value: 'pages/more/travelNotesIntroduction/travelStrategyDetails/index',
          label: '游记攻略'
        },
        {
          value: 'pages/more/boutiqueRoute/routerDetail/index',
          label: '精品路线'
        }
      ],
      contentSelect: [],
  ...
  methods: {
    ...
    typeChange() {
      if (this.form.name == 'pages/more/attractions/attractionsDetail/index') {
        this.isJumpLoading = true
        queryHotScenicSpotList().then(res => {
          this.contentSelect = res.data
          this.isJumpLoading = false
          console.log(res)
        })
      } else if (this.form.name == 'pages/more/deliciousFood/foodDetails/index') {
        const params = {
          page: 1,
          size: 100
        }
        ...
        getManagerList(params).then(res => {
          this.contentSelect = res.data.rows
          this.isJumpLoading = false
        })
      } else if (this.form.name == 'pages/index/gourmetSpecialty/foodDetails/index') {
        const params = {
          page: 1,
          size: 100
        }
        this.isJumpLoading = true
        pageQuerySpecialty(params).then(res => {
          this.contentSelect = res.data.rows
          this.isJumpLoading = false
        })
      } else if (this.form.name == 'pages/more/travelNotesIntroduction/travelStrategyDetails/index') {
        const params = {
          page: 1,
          size: 100
        }
        const data = {}
        this.isJumpLoading = true
        pageQueryTravel(params, data).then(res => {
          this.contentSelect = res.data.rows
          this.isJumpLoading = false
        })
      } else if (this.form.name == 'pages/more/boutiqueRoute/routerDetail/index') {
        const params = {
          page: 1,
          size: 100
        }
        const data = {}
        this.isJumpLoading = true
        pageQueryPath(params, data).then(res => {
          this.contentSelect = res.data.rows
          this.isJumpLoading = false
        })
      }
