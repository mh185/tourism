<template>
  <el-card>
    ...
    >
      ...
      <el-table-column prop="status" label="操作" width="400">
        <template slot-scope="scope">
          ...
          <!-- <el-button type="info" size="mini" @click="imageText(scope.row)">图文直播</el-button> -->
    ...
    <el-dialog title="查看大会直播" :visible.sync="lookCommodityStatus" width="50%">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="lookClear()">关 闭</el-button>
      </div>
    </el-dialog>
  </el-card>
</template>
...
import { pageQuery, deleteId, insert, update, upfiles, findById } from '@/api/playback'
export default {
  name: 'DropzoneDemo',
  ...
  methods: {
    ...
    lookClear() {
      this.form = {}
      this.lookCommodityStatus = false
    },
    // 大会直播查询
    query() {
      this.getList()
